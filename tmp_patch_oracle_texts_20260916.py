...
start=s.index('const CARD_ZOOM_RULES='); end=s.index('function openOfficialCard',start); block=s[start:end]
repls={
...
for old,new in repls.items(): assert old in block; block=block.replace(old,new,1)
...
"""
s=s[:start]+new+s[end:]
old="$('oracleNoticeTitle').textContent='ORACLE ACTIVÉ — '+name.toUpperCase();\n $('oracleNoticeText').textContent=oracleEffectText(name);"
new2="$('oracleNoticeTitle').textContent=oracleNoticeTitle(name);\n $('oracleNoticeText').innerHTML=oracleEffectText(name);"
